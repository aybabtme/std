...
// UseLog makes the retrier log retries to the given log.Log.
func UseLog(ll log.Log) Option {
	return func(r *retrier) { r.ll = ll }
}
...
func Do(ctx context.Context, fn func(context.Context) (bool, error), opts ...Option) error {
	...
	for {
		...
		retrier.ll.KV("retry.attempt", attempt).Err(err).Warn("retry attempt failed")
		if !retryFunc() {
			return err
		}
	}
